...
class PocCsvParserTest extends TestBase {
  ...
  "PocCsvParser" should {
    ...
    "throw a HeaderCsvException if row length is not enough" in {
      ...
      assert(result.size == 1)
      assert(result.head.left.get == s"""${pocId.toString};ub_vac_app;pocName;pocStreet;101;;12636;Wunschstadt;Wunschkreis;Wunschland;Deutschland;+4974339296;TRUE;;FALSE;certification-vaccination;Musterfrau;Frau;frau.musterfrau@mail.de;{"vaccines":["vaccine1", "vaccine2"]};the number of column 20 is invalid. should be 21.""")
    }

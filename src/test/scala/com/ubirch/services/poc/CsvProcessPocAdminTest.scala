    ...
    "fail to create poc, admin and status when the header is invalid" in {
      withInjector { injector =>
        ...
        (for {
          ...
          result.left.get shouldBe "poc_type_invalid* didn't equal expected header poc_type*; the right header order would be: external_id*,poc_type*,poc_name*,street*,street_number*,additional_address,zipcode*,city*,county,federal_state,country*,phone*,logo_url,manager_surname*,manager_name*,manager_email*,manager_mobile_phone*,extra_config"
          assert(pocs.isEmpty)

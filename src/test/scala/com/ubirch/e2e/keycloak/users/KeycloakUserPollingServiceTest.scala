    ...
    "React to changes of confirmation_mail_sent attribute and stop polling users once it is set to true" in {
      withInjector { injector =>
        ...
        val pollingResult = for {
          ...
          _ <- setConfirmationMailSentAttribute(value = true, keycloakUser1.userName)(userService, usersKeycloakConnector)
          _ <- setConfirmationMailSentAttribute(value = true, keycloakUser3.userName)(userService, usersKeycloakConnector)
          pollingResult <- pollingFiber.join
        } yield pollingResult

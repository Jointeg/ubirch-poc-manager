...
(
    ...
    PRIMARY KEY (id),
    CONSTRAINT unique_poc_combination UNIQUE (tenant_id, external_id, poc_type)
);
...
(
    ...
    poc_type_group_created      boolean,
    ...
    assigned_trusted_poc_group  boolean NOT NULL,
...
(
    ...
    device_creation_token     text,
    certify_group_id          text,
    device_group_id           text,
    tenant_type_group_id      varchar(36),
...
(
    ...
    web_authn_disconnected timestamp,
    status                 varchar(10) NOT NULL,
...
(
    ...
    invited_to_static_team_drive boolean,
    error_message                text,
    last_updated                 text    NOT NULL,
    created                      text    NOT NULL,
...
CREATE INDEX idx_poc_table_poc_name
    ON poc_manager.poc_table (poc_name);

CREATE INDEX idx_poc_table_poc_city
    ON poc_manager.poc_table (city);

CREATE INDEX idx_poc_admin_table_name
    ON poc_manager.poc_admin_table (name);

CREATE INDEX idx_poc_employee_table_name
    ON poc_manager.poc_employee_table (name);

CREATE INDEX idx_poc_admin_table_certify_user_id
    ON poc_manager.poc_admin_table (certify_user_id);

CREATE INDEX idx_poc_employee_table_certify_user_id
    ON poc_manager.poc_employee_table (certify_user_id);
